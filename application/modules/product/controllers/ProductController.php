...
//
//		} else {
$data['category_lists']= $this->MainModel->getAllData('', 'category', 'category_id,category_title', 'category_id ASC');

		if($this->input->post()){

			$category_id=$this->input->post('category_id');
			$data['category_id']=$this->input->post('category_id');
			 $query="SELECT product_name,product_price,created_time,purchase_price,status,product.product_id,purchase_price,discount_price,sku,product_stock,product_title  FROM `product` join term_relation on term_relation.product_id=product.product_id  WHERE term_relation.term_id =$category_id ";
			$data['products']=get_result($query);
			$data['pageContent'] = $this->load->view('product/products/products_index2', $data, true);
...
//			$installment_period				=	$this->input->post('installment_period');
    ...
    public function single_commission($id){
       
        $product_commision_exit_id  = $this->MainModel->getSingleData('product_id', $id, 'product_commission', '*');
            $product_name  = $this->MainModel->getSingleData('product_id', $id, 'product', '*');
          if($product_commision_exit_id){
                $this->session->set_flashdata('error', 'Commision for this product already been added');
                    redirect('product-list');
          }
           
             $data['product_id'] = $id;
              $data['product_name'] = $product_name->product_title; 
                 $data['commission'] = $this->ProductModel->all_commision_results(12, 1);
                $data['pageContent'] = $this->load->view('product/commission/commision_index', $data, true);
            $this->load->view('layouts/main', $data);
    }
    ...
    public function all_commission()
    {
        ...
        $this->load->library("pagination");
        $config = array();
        $config["base_url"] = base_url() . "product/ProductController/all_commission";
        $search_item = $this->input->post('product');
         if ($search_item) {
             $config["total_rows"]  = $this->ProductModel->all_commision_search_rows($search_item);
        }  
        else {
            $config["total_rows"]  = $this->ProductModel->all_commision_rows();
            $data['total_product']=$this->ProductModel->all_commision_rows();
        }
        
       $data['total_zero_product']=$this->ProductModel->all_zero_commision_rows();
        ...
        $page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        if ($search_item) {
            $data['commission'] = $this->ProductModel->all_commision_results_search($config["per_page"], $page, $search_item);
        } 
         else {
            $data['commission'] = $this->ProductModel->all_commision_results($config["per_page"], $page);
        }
        $data["links"] = $this->pagination->create_links();
        if ($this->input->is_ajax_request()) {
            $this->load->view('product/commission/commision_per_page', $data);
        } else {
            $data['pageContent'] = $this->load->view('product/commission/commision_index', $data, true);
            $this->load->view('layouts/main', $data);
        }
    ...
    public function create_commission()
    {
        ...
        if ($id) {
            ...
            $result = $this->MainModel->updateCommission($data, $id);
            if ($result) {
            ...
            $result = $this->MainModel->insertData('product_commission', $data);
            if ($result) {
