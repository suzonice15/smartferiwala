...
class MediaController extends MX_Controller
{
	...
	public function index()
	{
	 ...
	 $this->config->load('pagination');
        ...
        $search_item = $this->input->post('media_title');
         if ($search_item) {
          
             
              $config["total_rows"]  = $this->MidiaModel->order_search_rows($search_item);
             
        }  
        else {
            $config["total_rows"]  = $this->MidiaModel->all_order_rows();
        ...
        if ($search_item) {
                        ...
                        $data['media'] = $this->MidiaModel->order_search_result($config["per_page"], $page, $search_item);

        } 
         else {
            $data['media'] = $this->MidiaModel->all_orders_lists($config["per_page"], $page);
        }
 ...
 ;
	...
	function pagination()
	{
		$this->load->model("OrderModel_affilator");
		$this->load->library("pagination");
		$config = array();
		$config["base_url"] = "#";
