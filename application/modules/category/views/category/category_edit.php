<div class="col-md-offset-0 col-md-12">
	...
	<div class="box box-success">
		...
		<div class="box-body">
			...
			<form method="POST" action="<?= base_url('category-update') ?>"  id="cat" enctype="multipart/form-data">
				<div class="box-body">
					<div class="form-group <?= form_error('category_title') ? 'has-error' : '' ?>">
						...
						<input type="text" class="form-control the_title" name="category_title"
							   value="<?= set_value('category_title') ? set_value('category_title') : isset($category->category_title) ? $category->category_title : NULL ?>">
					</div>

					<div class="form-group <?= form_error('category_name') ? 'has-error' : '' ?>">
						<label for="category_name">Parmalink<span class="required">*</span></label>
						<input type="text" class="form-control the_name" name="category_name"
							   value="<?= set_value('category_name') ? set_value('category_name') : isset($category->category_name) ? $category->category_name : NULL ?>">
					...
					<div class="form-group <?= form_error('parent_id') ? 'has-error' : '' ?>">
						...
						foreach ($categories as $cat) {
							if (set_value('parent_id') == $cat->category_id) {
								$selected = 'selected';
							} elseif (isset($category->parent_id) && $category->parent_id == $cat->category_id) {
								$selected = 'selected';
							} else {
								$selected = NULL;
							}
					...
					<div hidden class="form-group featured-image">
						<label>Category Icon <span class="required"> (Size 30 * 30)</span></label>
						<?php
					...
					<div  class="form-group featured-image">
						<label>Banner Image<span class="required"> Size(318 * 624)</span></label>
						<?php
					...
					<div hidden class="form-group <?= form_error('target_url1') ? 'has-error' : '' ?>">
						<label for="target_url1">Gallery Target URL 01</label>
						<input type="text" class="form-control" name="target_url1"
							   value="<?= set_value('target_url1') ? set_value('target_url1') : isset($category->target_url1) ? $category->target_url1 : NULL ?>">
					</div>

					<div hidden class="form-group <?= form_error('target_url2') ? 'has-error' : '' ?>">
						<label for="target_url2">Gallery Target URL 02</label>
						<input type="text" class="form-control" name="target_url2"
							   value="<?= set_value('target_url2') ? set_value('target_url2') : isset($category->target_url2) ? $category->target_url2 : NULL ?>">
					</div>

					<div  hidden class="form-group <?= form_error('target_url3') ? 'has-error' : '' ?>">
						<label for="target_url3">Gallery Target URL 03</label>
						<input type="text" class="form-control" name="target_url3"
							   value="<?= set_value('target_url3') ? set_value('target_url3') : isset($category->target_url3) ? $category->target_url3 : NULL ?>">
					</div>
					...
					<div class="form-group <?= form_error('rank_order') ? 'has-error' : '' ?>">
						...
						<input type="text" class="form-control" name="rank_order"
							   value="<?= set_value('rank_order') ? set_value('rank_order') : isset($category->rank_order) ? $category->rank_order : NULL ?>">
					</div>
...
<div class="form-group">
    ...
    <select  name="status" id="payment_type" class="form-control">
        ...
        <option value="0">Unpublished </option>
					...
					<div hidden class="form-group">
						<div class="checkbox">
							<label><input type="checkbox" name="top_menu"
										  value="1" <?= ($category->top_menu == 1) ? 'checked' : null; ?>> Checked as
								Top Menu</label>
						</div>
					</div>
						...
						<div class="box-body">
							<div class="form-group <?= form_error('seo_title') ? 'has-error' : '' ?>">
								...
								<input type="text" class="form-control" name="seo_title" id="seo_title"
									   value="<?php if (set_value('seo_title')) {
										   echo set_value('seo_title');
									   } else {
										   echo $category->seo_title;
									   } ?>">
							</div>

							<div class="form-group <?= form_error('seo_meta_title') ? 'has-error' : '' ?>">
								<label for="seo_meta_title">SEO Meta Title</label>
								<input type="text" class="form-control" name="seo_meta_title" id="seo_meta_title"
									   value="<?php if (set_value('seo_meta_title')) {
										   echo set_value('seo_meta_title');
									   } else {
										   echo $category->seo_meta_title;
									   } ?>">
							</div>
							...
							<div class="form-group <?= form_error('seo_keywords') ? 'has-error' : '' ?>">
								...
								<input type="text" class="form-control" name="seo_keywords" id="seo_keywords"
									   value="<?php if (set_value('seo_keywords')) {
										   echo set_value('seo_keywords');
									   } else {
										   echo $category->seo_keywords;
									   } ?>">
							</div>
							...
							<div class="form-group <?= form_error('seo_content') ? 'has-error' : '' ?>">
								...
								<textarea class="form-control" rows="5" name="seo_content"
										  id="seo_content"><?php if (set_value('seo_content')) {
										echo set_value('seo_content');
									} else {
										echo $category->seo_content;
									} ?></textarea>
							</div>

							<div class="form-group <?= form_error('seo_meta_content') ? 'has-error' : '' ?>">
								<label for="seo_meta_content">SEO Meta Content</label>
								<textarea class="form-control" rows="5" name="seo_meta_content"
										  id="seo_meta_content"><?php if (set_value('seo_meta_content')) {
										echo set_value('seo_meta_content');
									} else {
										echo $category->seo_meta_content;
									} ?></textarea>
							</div>
						</div>
					</div>
	...
	<script>
		...
		document.forms['cat'].elements['payment_type'].value = "<?php echo $category->status;?>";
