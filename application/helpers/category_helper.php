...
function get_subcategories_web($category_id)
{
	$ci=get_instance();
	$ci->db->select('category_name,category_title');
	$ci->db->from('category');
	$ci->db->where('parent_id', $category_id);
	$ci->db->where('status', 1);
	$query=$ci->db->get();
...
function get_category_info_top($category_id){
   	$ci=get_instance();
...
{
	$ci=get_instance();
	$ci->db->select('category_title,category_name,medium_banner,category_id');
	$ci->db->from('category');
	$ci->db->where('category_id', $category_id);
	$query=$ci->db->get();
	if($query->num_rows() > 0)
...
function get_category_products($category_id, $limit, $initial_limit=NULL)
{
	...
	$ci=get_instance();
	$ci->db->select('product_percent_tag,product_name,product.product_id,product_title,product_price,discount_price,sku,product_stock,discount_type,product_availability');
	$ci->db->from('product');
	$ci->db->join('term_relation', 'product.product_id = term_relation.product_id');
	$ci->db->where('term_relation.term_id', $category_id);
	$ci->db->where('product.product_type', 'home');
...
/*# get category products #*/
function get_top_category_products($category_id, $limit, $initial_limit=NULL)
{

	$ci=get_instance();
	$ci->db->select('product_name,product.product_id,product_title,product_price,discount_price,sku,product_stock,discount_type,product_availability');
	$ci->db->from('product');
	$ci->db->join('term_relation', 'product.product_id = term_relation.product_id');
	$ci->db->where('term_relation.term_id', $category_id);
